...
def greetPerson():
    ...
    while True:
        if speak == True:
            myOutput=gTTS(text=person, lang='en', slow=False)
            myOutput.save('output.mp3')
            os.system('mpg123  output.mp3')
            speak = False
x=threading.Thread(target=greetPerson, daemon=True)
x.start()
...
scaleFactor = .25
# Modify for faster servo movement (higher number => more stable but slower)
servoScaleFactor = 25
flip = 2
fpsReport = 0
